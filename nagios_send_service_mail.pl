...
# Version : 1.9 FIXED: RRD file handling for nagiosgraph directory structure
# Version : 1.9.1 FIXED: URL encoding to match nagiosgraph (%20 not +)
# Version : 1.9.2 FIXED: Uninitialized $link_id warning when no graph found
# Version : 1.9.3 FIXED: Center alignment for table and graph
#                                                                       #
...
my $mail_sender        = "Nagios Monitoring <fim\@itcbd.com>";
#my $mail_sender        = "Nagios Monitoring <noreply\@ealert.meghnabank.com.bd>";

# The Nagios CGI URL for integrated service links
#my $nagios_cgiurl      = "http://nagios.fm4dd.com/nagios/cgi-bin";
my $nagios_cgiurl      = "http://w4-nagios.itcbd.com/nagios/cgi-bin";
...
my $html_style         = "body {text-align: center; font-family: Verdana, sans-serif; font-size: 10pt;}\n"
                       ...
                       . "table {margin-left: auto; margin-right: auto; border: 1px solid black; width: 600px;}\n"
                       ...
                       . "th,td {font-family: Verdana, sans-serif; font-size: 10pt; text-align:center;}\n"
                       . "th.customer {width: 600px; background-color: #004488; color: #ffffff; text-align:center;}\n"
                       . "p.foot {width: 600px; background-color: #004488; color: #ffffff; "
                       . "margin-left: auto; margin-right: auto; text-align:center;}\n"
                       . "div.graph-container {width: 600px; margin-left: auto; margin-right: auto; text-align:center;}\n"
                       . "img.graph {display: block; margin-left: auto; margin-right: auto;}\n";
my $table_size         = "600px";
...
my $test_service       = "Current Load"; # existing services in Nagios
...
my $ngraph_cgiurl      = "http://w4-nagios.itcbd.com/nagios/cgi-bin/show.cgi";
...
my $domain 	       = "\@itcbd.com"; # this is only for -g groups
my @listaddress        = ();
...
# ########################################################################
#my $logofile = "/var/www/html/fm4dd/images/nagios-mail.gif";
my $logofile = "/var/www/html/fm4dd/images/qcash.png";
...
my %language = ('en' => { 'A' => 'Customer',
                ...
                'es' => { 'A' => 'Cliente',
                          ...
                          'E' => 'Grupo del Servicio',
                          'F' => 'Datos del Servicio',
                          'G' => 'Nombre del Equipo',
                          'H' => 'Alias del Equipo',
                          'I' => 'Dirección IP',
                          'J' => 'Grupo de Equipos',
                          'K' => 'Momento del Evento',
                          'L' => 'Autor',
                          'M' => 'Comentarios',
                          'N' => 'Notificacion del Sistema de Monitoreo Nagios',
                          'O' => 'Generado por Nagios, la solución de monitoreo/seguimiento de Código Abierto.' },
             'es-en' => { 'A' => 'Cliente (Customer)',
                          'B' => 'Tipo de Notificación (Notification Type)',
                          'C' => 'Nombre del Servicio (Service Name)',
                          'D' => 'Estado del Servicio (Service Status)',
                          'E' => 'Grupo del Servicio (Service Group)',
                          'F' => 'Datos del Servicio (Service Data)',
                          'G' => 'Nombre del Equipo (Hostname)',
                          'H' => 'Alias del Equipo (Hostalias)',
                          'I' => 'Dirección IP (IP Address)',
                          'J' => 'Grupo de Equipos (Hostgroup)',
                          'K' => 'Momento del Evento (Event Time)',
                          'L' => 'Autor (Author)',
                          'M' => 'Comentarios (Comment)',
                          'N' => 'Notificacion del Sistema de Monitoreo Nagios (Nagios Monitoring System Notification)',
                          'O' => 'Generado por Nagios, la solución de monitoreo/seguimiento de Código Abierto (Generated by Nagios, the OpenSource monitoring solution)' });
...
sub create_message_html {
  ...
  if ($o_format eq "multi" || $o_format eq "graph") {
    ...
    $html_msg .= "<td colspan=\"2\"><span>$language{$land}{'N'}</span></td></tr><tr>\n"; }
  ...
  if (defined($o_addurl)) {
    ...
    if ($o_format eq "graph" && $graph_type ne "gif") {
      ...
      $html_msg .= " <a href=\"$ngraph_cgiurl?host=" . urlencode($o_hostname)
                .  "&service=" . urlencode($o_servicedesc) . "&geom=". $graph_img_size ."\">"
  ...
  if (defined($graph_img) && $o_format eq "graph") {
    ...
    $html_msg .= "<br><div class=\"graph-container\"><img class=\"graph\" src=\"cid:$graph_id\"></div>\n";
  }
...
# #######################################################################
# urlencode() URL encode a string to match nagiosgraph's encoding
# #######################################################################
sub urlencode {
  my $urldata = $_[0];

  # First, encode special characters the way nagiosgraph does
  $urldata =~ s/([^a-zA-Z0-9_.-])/sprintf("%%%02X", ord($1))/eg;
...
sub dbfilelist {
  ...
  my $host_dir = $rrddir . "/" . $host;

  # URL encode the service name to match nagiosgraph file naming
  my $encoded_service = urlencode($service);
  $hs = $encoded_service . "___";

  verb("sub dbfilelist: Checking files inside directory: ".$host_dir." looking for prefix: ".$hs);

  my @rrd = ();

  # Check if directory exists
  if (! -d $host_dir) {
    verb("sub dbfilelist: Directory $host_dir does not exist");
    return @rrd;
  }

  # Open directory safely
  if (!opendir DH, $host_dir) {
    verb("sub dbfilelist: Cannot open directory $host_dir - $!");
    return @rrd;
  }

  # Read directory and filter files matching the pattern
  my @files = readdir DH;
  closedir DH;

  # Filter files that match the service pattern
  foreach my $file (@files) {
    if ($file =~ /^${hs}(.+)\.rrd$/) {
      push @rrd, $1;
      verb("sub dbfilelist: Found data source: $1 from file: $file");
    }
  }

  verb("sub dbfilelist: We found number of data sources: ".scalar(@rrd));
  return @rrd;
}
...
sub graphinfo {
  ...
  # Build the full path to the host directory
  my $host_dir = $rrddir . "/" . $host;

  verb("sub graphinfo: Looking for files in: $host_dir with prefix: $hs");

  # Determine which files to read lines from
  my @ds_list = dbfilelist($host, $service, $rrddir);

  # Build the file list with proper paths
  @rrd = map {{
    file => $host . "/" . $hs . $_ . ".rrd"
  }} @ds_list;
  ...
  if (@rrd < 1) {
    verb("sub graphinfo: Could not find any graph file for $host/$service");
    return undef;
  } else {
    verb("sub graphinfo: Found " . scalar(@rrd) . " graph files for $host/$service");
  }

  for $f ( @rrd ) {
    unless ( $f->{line} ) {
      my $full_path = $rrddir . "/" . $f->{file};
      verb("sub graphinfo: Reading RRD info from: $full_path");
      $ds = RRDs::info $full_path;
      if (RRDs::error) {
        verb("sub graphinfo: RRDs::info ERR " . RRDs::error);
        next;
      }
    ...
    verb("sub graphinfo: DS $f->{file} lines: " . join ', ', keys %{ $f->{line} } );
  }
  return \@rrd;
}
...
sub rrdline {
  ...
  for $g ( @$G ) {
    ...
    $longest = 10 unless defined($longest); # Default if no lines found

    for $v ( sort keys %{ $g->{line} } ) {
      $c = hashcolor($v);
      verb("rrdline: file=$f line=$v color=$c");
      my $sv = "$v";
...
sub create_graph_image {
  ...
  verb("create_graph_image: Attempting to create graph for $o_hostname/$o_servicedesc");
  verb("create_graph_image: RRD base directory: $rrd_basedir");
  ...
  if(! defined($G)) {
    verb("create_graph_image: Cannot generate the Nagiosgraph image file - no RRD data found");
  ...
  RRDs::graph(@ds);
  if (RRDs::error) {
    verb("create_graph_image: RRDs::graph ERR " . RRDs::error);
    $graph_type = "gif";
    $graph_img = $empty_img;
    return $graph_img;
  }
...
if ($o_format eq "multi" || $o_format eq "graph") {
  ...
  if (defined($graph_img) && $o_format eq "graph") {
    ...
    # create the third boundary marker for the graph link image (only if link_id is defined)
    if (defined($link_id)) {
        $mail_content = $mail_content . "$boundary\n"
